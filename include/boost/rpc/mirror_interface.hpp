  ...
  #ifndef BOOST_RPC_MIRROR_INTERFACE_HPP
  #define BOOST_RPC_MIRROR_INTERFACE_HPP
  ...
  namespace boost { namespace rpc {
     ...
     *         reflected and transforms it to add RPC functionality.
     *
     *  Specific transformations include:
     *  
     *  - Converting all return types to futures, RPC is inheriently async.
     *  - Adding calling conventions
     *      - oneway (no return value)
     *      - gauranteed/non-gauranteed
...
#        ifndef BOOST_RPC_IMPL_SIZE
#           define BOOST_RPC_IMPL_SIZE 8
#        endif

#       include <boost/preprocessor/iteration/iterate.hpp>
#       define BOOST_PP_ITERATION_LIMITS (0, BOOST_RPC_IMPL_SIZE -1 )
#       define BOOST_PP_FILENAME_1 <boost/rpc/mirror_interface.hpp>
#       include BOOST_PP_ITERATE()
  ...
  #endif // BOOST_RPC_MIRROR_INTERFACE_HPP
...
struct mirror_member<R(Class::*)(PARAM_TYPES)const> {
  typedef boost::cmt::future<R>                                               result_type;
...
{
  typedef boost::cmt::future<typename boost::reflect::adapt_void<R>::result_type> result_type;
  typedef mirror_member                                                           self_type;
  typedef boost::fusion::vector<PARAM_TYPES>                                      fused_params;
  typedef boost::function_traits<result_type(PARAM_TYPES)>                        traits;
  ...
  result_type operator()( PARAM_ARGS ) {
    return (*this)( fused_params(PARAM_NAMES) );
  }
  ...
  typename boost::reflect::adapt_void<R>::result_type emit( const fused_params& fp ) {
    scoped_block_signal block_reverse(m_reverse_con);
  ...
  mirror_member& operator=( const T& d )  {
    ...
    m_reverse_con = 
        d.connect( boost::fusion::make_unfused( 
                            boost::bind( &mirror_member::emit, this, _1) )  );
    m_delegate = emit_or_throw(boost::reflect::adapt_void<R,T>(d));
                            */
    return *this;
  }

...
namespace boost { namespace rpc { namespace json { namespace io {
    ...
    namespace detail {
      ...
      struct if_reflected {
        ...
        static inline void pack( Filter& f,json::value& s, const T& v ) { 
          ...
          // TO BASE 64
          json::io::pack(f,s,f(boost::rpc::base64_encode((unsigned char const*)ss.str().c_str(),ss.str().size())));
        }
        template<typename T,typename Filter>
    ...
    inline void pack( Filter& f, json::value& jsv, const std::vector<char>& data ) {
       if( data.size() ) { pack( f, jsv, f(boost::rpc::base64_encode((unsigned char*)&data.front(),data.size()))); } 
    }
    template<typename Filter>
